...
@router.callback_query(lambda c: c.data is not None and c.data.startswith("sub_"))
async def cb_subscribe(callback: CallbackQuery) -> None:
    ...
    async with get_db_session() as session:
        ...
        if not isinstance(message, Message):
            await callback.answer("❌ Message not accessible")
            return
...
@router.callback_query(lambda c: c.data is not None and c.data == "format_digest")
...
@router.callback_query(lambda c: c.data is not None and c.data == "format_simple")
...
@router.callback_query(lambda c: c.data is not None and c.data == "format_concepts")
